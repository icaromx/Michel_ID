...
int main(int argc, char **argv){
	...
	double dist_unclustered = 10.; //Distance at which the unclustered point is considered for the unclustered/clustered ratio
	...
	TNtuple *nt_ddx_cl = new TNtuple("nt_ddx_cl","nt_ddx_cl","run_num:ev_num:cluster_id:rr:dQdx:dEdx:t_dQdx:t_dEdx");
	TNtuple *nt_cluster_info = new TNtuple("nt_cluster_info","nt_cluster_info","run_num:ev_num:cluster_id:el_energy:mu_energy:vertex_res:sp_vertex_res");
		...
		for (int c = 0; c < num_clusters; ++c){
			...
			if(is_selected){
				...
				for(int i = 0; i < Cl.size(); ++i){
					...
					if(i > 0) track_range = track_range - dx;
					if(i > q_win && i < Cl.size() - q_win){
						for(int j = i - q_win; j < i + q_win; ++j){
						...
						dQdx = dQ/dx;
						dEdx = dE/dx;
						nt_ddx_cl -> Fill(run_num,ev_num,cluster,track_range,dQdx,dEdx);
						dQ = 0.;
						dQ = 0.;
						dx = 0.;
						v_q.clear();
				...
				nt_cluster_info -> Fill(run_num,ev_num,cluster,el_energy,mu_energy,vertex_res,sp_vertex_res);	
			}		    	
		}
		infile->Close();
	}
